...
const fetchReportDataWeeklyCampaignNB = async (dateRanges) => {
...
const fetchReportDataWeeklyNBFilter = async (req, res, campaignNameFilter, brandNBFilter, dateRanges) => {
  const refreshToken_Google = getStoredGoogleToken();
...
const createFetchFunction = (campaignNameFilter, brandNBFilter = "") => {
  return (req, res, dateRanges) => fetchReportDataWeeklyNBFilter(req, res, campaignNameFilter, brandNBFilter, dateRanges);
};

const fetchFunctions = {
  fetchReportDataWeeklyNBSearchBrand: createFetchFunction("Search_Brand", ""),
  fetchReportDataWeeklyNBSearchNB: createFetchFunction("Search_NB", ""),
  fetchReportDataWeeklyNBSearchNBElPaso: createFetchFunction("Search_NB_El-Paso", ""),
};

const executeSpecificFetchFunctionNB = async (req, res) => {
  const functionName = "fetchReportDataWeeklyNBSearchNB";
  const dateRanges = getOrGenerateDateRanges();
...
const throttledFetchFunctions = {
  weeklyCampaignData: createThrottledFetch(fetchReportDataWeeklyCampaignNB),
  shoppingBrandData: createThrottledFetch(fetchFunctions.fetchReportDataWeeklyNBSearchBrand),
  shoppingNBData: createThrottledFetch(fetchFunctions.fetchReportDataWeeklyNBSearchNB),
  elPasoData: createThrottledFetch(fetchFunctions.fetchReportDataWeeklyNBSearchNBElPaso),
};

const sendFinalWeeklyReportToGoogleSheetsNB = async (req, res) => {
  ...
  const spreadsheetId = process.env.SHEET_NB;
  const dataRanges = {
  ...
  try {
    ...
    const {
      ...
      shoppingBrandData: throttledSearchBrandDataFetch,
      shoppingNBData: throttledSearchNBDataDataFetch,
      elPasoData: throttledSearchNBElPasoDataFetch,
    } = throttledFetchFunctions;

    const weeklyCampaignData = await throttledWeeklyCampaignDataFetch(dateRanges);
    const searchBrandData = await throttledSearchBrandDataFetch(req, res, dateRanges);
    const searchNBData = await throttledSearchNBDataDataFetch(req, res, dateRanges);
    const elPasoData = await throttledSearchNBElPasoDataFetch(req, res, dateRanges);
    ...
    addDataToRecords(searchBrandData, "Search Brand", 2);
    addDataToRecords(searchNBData, "Search NB", 3);
    addDataToRecords(elPasoData, "Search NB El Paso", 4);

    if (!date || date.trim() === '') {
      addWoWVariance(weeklyCampaignData.slice(-2)[0], weeklyCampaignData.slice(-3)[0], "All Campaign", 1);
      addWoWVariance(searchBrandData.slice(-2)[0], searchBrandData.slice(-3)[0], "Search Brand", 2);
      addWoWVariance(searchNBData.slice(-2)[0], searchNBData.slice(-3)[0], "Search NB", 3);
      addWoWVariance(elPasoData.slice(-2)[0], elPasoData.slice(-3)[0], "Search NB El Paso", 4);
    }
    ...
    if (!date || date.trim() === '') {
      ...
      addBiWeeklyVariance(searchBrandData.slice(-2)[0], searchBrandData.slice(-3)[0], searchBrandData.slice(-4)[0], searchBrandData.slice(-5)[0], "Search Brand", 2);
      addBiWeeklyVariance(searchNBData.slice(-2)[0], searchNBData.slice(-3)[0], searchNBData.slice(-4)[0], searchNBData.slice(-5)[0], "Search NB", 3);
      addBiWeeklyVariance(elPasoData.slice(-2)[0], elPasoData.slice(-3)[0], elPasoData.slice(-4)[0], elPasoData.slice(-5)[0], "Search NB El Paso", 4);
    }
    records.sort((a, b) => a.Filter2 - b.Filter2);
    ...
    const dataToSend = {
      Live: sheetData.filter(row => ["All Campaign", "Search Brand", "Search NB", "Search NB El Paso"].includes(row[0]) || ["All Campaign", "Search Brand", "Search NB", "Search NB El Paso"].includes(row[1])),
    };    
