...
const fetchFunctions = {
  fetchReportDataWeeklyTotal: createFetchFunction(process.env.GOOGLE_ADS_CUSTOMER_ID_FLX, "Total"),
};

const executeSpecificFetchFunctionFLX = async (req, res) => {
...
const sendFinalWeeklyReportToGoogleSheetsFLX = async (req, res) => {
  ...
  const spreadsheetId = process.env.SHEET_FLX;
  const dataRanges = {
    FLXLive: 'Weekly Report!A2:K',
  };
  ...
  try {
    ...
    const dataToSend = {
      FLXLive: sheetData.filter(row => ["All Campaign"].includes(row[1])),
    };    
    ...
    console.log("Final Flexfits weekly report sent to Google Sheets successfully!");
  } catch (error) {
...
module.exports = {
  executeSpecificFetchFunctionFLX,
